...
export interface SavedDesign { id: number; user_id?: number; name: string; params: Params; created_at: number; nickname?: string; rating?: Rating; }
...
export interface RankedDesign { id: number; user_id: number; name: string; nickname: string; avatar_url: string; average: number; ratings: number; }
...
export interface ProfileDesign { id: number; name: string; created_at: number; nickname: string; rating: Rating; }
...
export async function saveDesign(name: string, params: Params): Promise<SavedDesign> {
  return accountRequest<SavedDesign>("/api/designs", { method: "POST", body: JSON.stringify({ name, params }) });
}
