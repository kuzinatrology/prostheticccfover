...
def save_design(user_id: int, name: str, params: dict[str, Any], motif_id: str, motif_data: bytes | None) -> dict[str, Any]:
    name = name.strip()[:100] or "Untitled design"
    db = _db()
    cur = db.execute("INSERT INTO designs(user_id,name,params,motif_id,motif_data,created_at) VALUES(?,?,?,?,?,?)", (user_id, name, json.dumps(params), motif_id, motif_data, int(time.time())))
    db.commit()
    return {"id": cur.lastrowid, "name": name, "params": params, "created_at": int(time.time())}


def list_designs(user_id: int) -> list[dict[str, Any]]:
    db = _db()
    return [{"id": r["id"], "name": r["name"], "params": json.loads(r["params"]), "created_at": r["created_at"], "rating": _rating(db, r["id"])} for r in db.execute("SELECT id,name,params,created_at FROM designs WHERE user_id=? ORDER BY created_at DESC", (user_id,))]
...
def community_designs(search: str = "", user_id: int | None = None, limit: int = 100) -> list[dict[str, Any]]:
    ...
    rows = db.execute("""
      SELECT d.id,d.user_id,d.name,d.created_at,u.nickname
      FROM designs d JOIN users u ON u.id=d.user_id
      WHERE d.name LIKE ? COLLATE NOCASE
      ORDER BY d.created_at DESC LIMIT ?
    """, (search, min(max(limit, 1), 200))).fetchall()
    return [{"id": r["id"], "user_id": r["user_id"], "name": r["name"], "nickname": r["nickname"], "created_at": r["created_at"], "rating": _rating(db, r["id"], user_id)} for r in rows]
...
def get_public_design(design_id: int, user_id: int | None = None) -> tuple[dict[str, Any], bytes | None] | None:
    ...
    if not r:
        return None
    design = {"id": r["id"], "user_id": r["user_id"], "name": r["name"], "nickname": r["nickname"], "params": json.loads(r["params"]), "created_at": r["created_at"], "rating": _rating(db, design_id, user_id)}
...
def top_designs(limit: int = 5) -> list[dict[str, Any]]:
    db = _db()
    rows = db.execute("""
      SELECT d.id,d.user_id,d.name,u.nickname,
        CASE WHEN u.avatar_data IS NOT NULL THEN '/api/profiles/' || u.id || '/avatar' ELSE u.avatar_url END AS avatar_url,
...
def profile(user_id: int, viewer_id: int | None = None) -> dict[str, Any]:
    ...
    designs = db.execute(
        "SELECT id,name,created_at FROM designs WHERE user_id=? ORDER BY created_at DESC",
        (user_id,),
    ).fetchall()
    design_items = [
        {
...
def get_design(user_id: int, design_id: int) -> tuple[dict[str, Any], bytes | None] | None:
    ...
    if not r:
        return None
    return {"id": r["id"], "name": r["name"], "params": json.loads(r["params"]), "created_at": r["created_at"]}, r["motif_data"]
